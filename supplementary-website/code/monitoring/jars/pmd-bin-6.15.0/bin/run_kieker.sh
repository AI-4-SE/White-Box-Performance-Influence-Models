...
jre_specific_vm_options() {
  ...
  then
    ...
    echo $options
  elif [ "${APPNAME}" = "pmd" ] || [ "${APPNAME}" = "cpd" ]
  then
    #options="-javaagent:/home/max/PhD/performance_measurement_scripts/monitoring/profile_lib/kieker-1.14-SNAPSHOT-aspectj.jar"
    #options="-javaagent:/home/max/PhD/performance_measurement_scripts/monitoring/profile_lib/kieker-1.13-aspectj.jar"
    options="-javaagent:profile_lib/kieker-1.13-aspectj_sloppy.jar"
    options+=" -Dkieker.monitoring.skipDefaultAOPConfiguration=true"
    options+=" -Dkieker.monitoring.configuration=${CONFIGPATH}"
    options+=" -Dorg.aspectj.weaver.loadtime.configuration=file:build/META-INF/aop.xml"
    echo $options
  else
    echo ""
  fi
...
java ${HEAPSIZE} ${PMD_JAVA_OPTS} $(jre_specific_vm_options) -cp "${classpath}" "${CLASSNAME}" $OUT
